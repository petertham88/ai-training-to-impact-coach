...
import { serviceDb, withRetry } from "@/lib/supabase/data";
...
// Session-bound client + the signed-in user. All participant writes run through
// this so rows are owned by the user and satisfy owner RLS once it is applied.
async function sessionCtx(): Promise<{
...
async function syncProblemStatus(
  workProblemId: string,
  client: SupabaseClient,
): Promise<void> {
  const p = await getWorkflowProgress(workProblemId, client);
  if (!p) return;
  const status = statusForStep(p.completedThrough);
  if (status !== p.problem.status) {
    await client.from("work_problems").update({ status }).eq("id", workProblemId);
  }
  if (p.useCase) {
    let ucStatus = "draft";
    ...
    if (ucStatus !== p.useCase.status) {
      await client.from("use_cases").update({ status: ucStatus }).eq("id", p.useCase.id);
    }
  }
}
...
export async function createWorkProblem(formData: FormData): Promise<void> {
  const { supabase, userId } = await sessionCtx();
...
export async function updateWorkProblem(formData: FormData): Promise<void> {
  const { supabase } = await sessionCtx();
...
export async function deleteWorkProblem(formData: FormData): Promise<void> {
  ...
  if (ucIds.length) {
    ...
    if (pbIds.length) {
      await supabase.from("outcomes").delete().in("playbook_id", pbIds);
      await supabase.from("playbooks").delete().in("id", pbIds);
    }
    await supabase.from("experiments").delete().in("use_case_id", ucIds);
    await supabase.from("prompts").delete().in("use_case_id", ucIds);
    await supabase.from("use_cases").delete().in("id", ucIds);
  }
  const { error } = await supabase.from("work_problems").delete().eq("id", id);
  if (error) throw error;
...
export async function saveUseCase(formData: FormData): Promise<void> {
  ...
  if (id) {
    const { error } = await supabase.from("use_cases").update(payload).eq("id", id);
    if (error) throw error;
    await logAudit({
  ...
  await syncProblemStatus(work_problem_id, supabase);
...
export async function setWinningPrompt(formData: FormData): Promise<void> {
  ...
  await supabase
    ...
    .eq("use_case_id", use_case_id)
    .eq("status", "winning");
  await supabase.from("prompts").update({ status: "winning" }).eq("id", id);
  await logAudit({
...
export async function savePlaybook(formData: FormData): Promise<void> {
  ...
  if (winning) {
    await supabase
      ...
      .eq("use_case_id", use_case_id)
      .eq("status", "winning");
    await supabase.from("prompts").update({ status: "winning" }).eq("id", winning);
  }
...
export async function recordOutcome(formData: FormData): Promise<void> {
  ...
  if (id) {
    const { error } = await supabase.from("outcomes").update(payload).eq("id", id);
    if (error) throw error;
    await logAudit({
...
// ── Cross-role writes (manager / trainer) ───────────────────────────────────
// These act on other participants' rows, so they use the service client
// (cross-user). Under applied owner-RLS they need SUPABASE_SERVICE_ROLE_KEY.
